            ...
            if (import.meta.env.VITE_MINI_APP_ENV === 'test') {
                ...
                if (existingUsers && existingUsers.length > 0) {
                    const referralRanking: ReferralRankingItem[] = existingUsers.map((user, /* index */) => {
                        ...
                        return {
                            rank: 0,
                            name: user.user_details.user_base.telegram_info.username == undefined ? user.user_details.user_base.telegram_info.telegram_id : user.user_details.user_base.telegram_info.username,
            ...
            if (import.meta.env.VITE_MINI_APP_ENV == 'test') {
                ...
                const pointRanking = existingUsers && await Promise.all(existingUsers.map(async (user) => {
                    ...
                    if (dbUser?.user_details.user_base.telegram_info.username) {
                        ...
                        return {
                            rank: user.rank,
                            name: dbUser?.user_details.user_base.telegram_info.username == undefined ? dbUser?.user_details.user_base.telegram_info.telegram_id : dbUser?.user_details.user_base.telegram_info.username,
                ...
                if (myPointRankingFromServer && account?.telegram_info.username && pointRanking) {
                    setMyPointRecord({
                        ...
                        name: account?.telegram_info.username == undefined ? account?.telegram_info.telegram_id : account?.telegram_info.username,
